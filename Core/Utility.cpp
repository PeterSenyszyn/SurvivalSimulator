...
{
	window->SetPosition(sf::Vector2f((Application::getCurrentResolution().x / 2) - (window->GetRequisition().x / 2), window->GetAbsolutePosition().y));
}
...
{
	std::uniform_int_distribution<> distr(minNum, maxNum - 1);
