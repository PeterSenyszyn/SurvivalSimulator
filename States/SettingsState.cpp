...
State(stack, context)
{
    ...
    tempMainWindow->SetPosition(sf::Vector2f(0, 200));

    /************************************************************************************************/
    /************************************************************************************************/

    auto generalBox = sfg::Box::Create(sfg::Box::Orientation::VERTICAL, 40.f);
    ...
    guiManager.setDesktopProperty(std::string("#resolutionPromptLabel"), std::string("FontSize"), 30.f);

    m_resolutionComboBox = sfg::ComboBox::Create();
    m_resolutionComboBox->SetId("resolutionComboBox");
    guiManager.setDesktopProperty(std::string("#resolutionComboBox"), std::string("FontSize"), 30.f);
    ...
    guiManager.setDesktopProperty(std::string("#refreshRateLabel"), std::string("FontSize"), 20.f);

    m_maxRefreshRate = sfg::ComboBox::Create();
    m_maxRefreshRate->SetId("maxRefreshRateComboBox");
    guiManager.setDesktopProperty(std::string("#maxRefreshRateComboBox"), std::string("FontSize"), 20.f);
    ...
    m_saveButton->SetRequisition(sf::Vector2f(200, 100));
    ...
    notebook->SetRequisition(sf::Vector2f(500, 75));
...
{
    sf::Vector2u res; //= parseResolution(resolutionCombo->getSelectedItem().toAnsiString());
    int x = res.x, y = res.y;
